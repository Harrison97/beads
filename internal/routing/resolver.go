...
// a subdirectory, which is useful when the town root itself is a repo.
...
// lookupRigForgivingWithTown is like LookupRigForgiving but also returns the town root.
func lookupRigForgivingWithTown(input, currentBeadsDir string) (Route, string, bool) {
	...
	// Load routes from town level (or local if no town)
	routes, townRoot := findTownRoutes(currentBeadsDir)
	if len(routes) == 0 {
		return Route{}, "", false
	}

	// Normalize prefix variants for matching
